defmodule TwimWeb.AuthController do
  ...
  defp get_token!("twitter", code),   do: Twitter.get_token!(code: code)
  defp get_token!("google", code),   do: Google.get_token!(code: code)
  defp get_token!("facebook", code), do: Facebook.get_token!(code: code)
  defp get_token!(_, _), do: raise "No matching provider available"

  defp get_user!("twitter", client) do
    %{body: user} = OAuth2.Client.get!(client, "/users/show", fields: "user_id")
    %{name: user["name"], avatar: user["profile_image_url"], token: client.token.access_token}
  end
